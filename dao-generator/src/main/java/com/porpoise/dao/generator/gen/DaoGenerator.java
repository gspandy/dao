...
public class DaoGenerator
{
	...
	public static void generatePom(final String artifactId, final String groupId, final String version,
			final File destDir) throws IOException
	{
		...
		else
		{
			...
			{
				throw new IllegalArgumentException("Couldn't create destination directory " + destDir);
			}
	...
	private static void doGenerate(final File destFolder, final DaoContext ctxt,
			final Map<String, IGenerator> templateByFilename) throws IOException
	{
	...
	private static void generate(final File destFolder, final IGenerator template, final DaoContext ctxt,
			final String javaFileName) throws IOException
	{
		...
		{
			throw new IllegalStateException("Could not create sub-package directory "
