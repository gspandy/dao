...
public class SqlTemplate implements IGenerator
{
  ...
  protected final String TEXT_9 = "\";" + NL + "" + NL + "    // cached (computing) maps for query strings" + NL + "    private static final ConcurrentMap<String, String> attributeListAsStringMap;" + NL + "    private static final ConcurrentMap<String, String> attributeListAsUpdateStringMap;" + NL + "    private static final ConcurrentMap<String, String> selectByAliasMap;" + NL + "" + NL + "\tstatic {" + NL + "\t\tattributeListAsStringMap = new MapMaker().initialCapacity(3)" + NL + "\t\t\t\t.makeComputingMap(new Function<String, String>() {" + NL + "\t\t\t\t\t@Override" + NL + "\t\t\t\t\tpublic String apply(final String aliasParam) {" + NL + "\t\t\t\t\t    assert aliasParam != null : \"alias was null\";" + NL + "\t\t\t\t\t    final String alias = emptyToNull(aliasParam);" + NL + "\t\t\t\t\t\treturn Field.asAttributeList(alias," + NL + "\t\t\t\t\t\t\t\t";
  protected final String TEXT_10 = "Metadata.FIELDS);" + NL + "\t\t\t\t\t}" + NL + "\t\t\t\t});" + NL + "" + NL + "\t\tattributeListAsUpdateStringMap = new MapMaker().initialCapacity(3)" + NL + "\t\t\t\t.makeComputingMap(new Function<String, String>() {" + NL + "\t\t\t\t\t@Override" + NL + "\t\t\t\t\tpublic String apply(final String aliasParam) {" + NL + "\t\t\t\t\t    assert aliasParam != null : \"alias was null\";" + NL + "\t\t\t\t\t    final String alias = emptyToNull(aliasParam);" + NL + "\t\t\t\t\t\treturn Field.asUpdateList(alias, ";
  protected final String TEXT_11 = "Metadata.FIELDS);" + NL + "\t\t\t\t\t}" + NL + "\t\t\t\t});" + NL + "\t\tselectByAliasMap = new MapMaker().initialCapacity(3).makeComputingMap(" + NL + "\t\t\t\tnew Function<String, String>() {" + NL + "\t\t\t\t\t@Override" + NL + "\t\t\t\t\tpublic String apply(final String aliasParam) {" + NL + "\t\t\t\t\t    assert aliasParam != null : \"alias was null\";" + NL + "\t\t\t\t\t    final String alias = emptyToNull(aliasParam);" + NL + "\t\t\t\t\t\treturn String.format(\"SELECT %s FROM %s %s\"," + NL + "\t\t\t\t\t\t\t\tattributeListAsString(alias), TABLE_NAME," + NL + "\t\t\t\t\t\t\t\taliasParam);" + NL + "\t\t\t\t\t}" + NL + "\t\t\t\t});" + NL + "\t}" + NL + "" + NL + "    /**" + NL + "     * @return a comma-separated list of ";
  protected final String TEXT_12 = " attributes in string form " + NL + "     */" + NL + "    public static String attributeListAsString(final String alias)" + NL + "    {" + NL + "        // allow null aliases as null has a meaning" + NL + "        return attributeListAsStringMap.get(nullToEmpty(alias));" + NL + "    }" + NL + "    " + NL + "    /**" + NL + "     * @return the ";
  protected final String TEXT_13 = " attributes in a comma-separated &quot;KEY-N=?&quot; form  " + NL + "     */" + NL + "    public static String attributeListAsUpdateString(final String alias)" + NL + "    {" + NL + "        // allow null aliases as null has a meaning" + NL + "        return attributeListAsUpdateStringMap.get(nullToEmpty(alias));" + NL + "    }" + NL;
