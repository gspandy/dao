...
/**
 * DbConnectionFactory created: Jul 27, 2010
 * 
 * @author Aaron
 *         <p>
 *         A DbConnectionFactory is used to register database connections. {@link DataSource}, {@link Connection}, and
 *         {@link IDbTransaction} objects can then be attained from this factory.
 *         </p>
 *         <p>
 *         The preferred access to a database is via an IDbTransaction, however, as that guarantees connections will be properly closed
 *         after use.
 *         </p>
 *         <p>
 *         Also, though the DbConnectionFactory is a singleton, classes should still require if possible for DbConnectionFactory instances
 *         to be passed in so as not to abuse this fact.
 *         </p>
 *         <p>
 *         A DbConnectionFactory will need to have datasources registered with it, and the preferred method of doing so is via an
 *         {@link IDatabaseVendor}. IDatabaseType implementations are available via the {@link Databases} object.
 *         </p>
 *         <p>
...
{
    /**
     * the singleton instance. As of java 1.5, the preferred singleton mechanism is to use them as an enumeration
     */
    INSTANCE;
    ...
    /**
     * <p>
     * Return the singleton factory instance. This usage is discouraged, however, as clients of the DbConnectionFactory should advertise
     * their dependence on it through their API.
     * </p>
     * <p>
    ...
    /**
     * register a datasource with the connection factory. If no default datasources have been registered, then this datasource will be used
     * as the default.
     * 
     * @param key
     ...
     * @return the datasource previously registered against that key or null if none were already registered.
     */
    public DataSource registerDatasource(final String key, final DataSource dataSource)
    {
        Preconditions.checkNotNull(key, "the datasource key cannot be null");
    ...
    /**
     * @param key
     *            the key for which a givne datasource is expected to be registered
    ...
    /**
     * return a database connection from a datasource registered with the given name (key)
     * 
     * @param name
    ...
    /**
     * Convenience method for attaining a database connection from the default datasource, instead of having to use:
     * 
     * <pre>
    ...
    public static Connection getDefaultConnection()
    {
        ...
        {
            throw new IllegalStateException("no datasources have been registered. Have you called init on an IDatabaseType?");
        }
        final DataSource defaultDataSource = getInstance().getDefaultDataSource();
        if (defaultDataSource == null)
        {
            throw new IllegalStateException(String.format("no datasources have been registered by the default name '%s'.",
    ...
    /**
     * start a new transaction using the datasource registered against the given key
     * 
     * @param key
    ...
    /**
     * convenience method for single-use queries to save the user having to always run queries within a try/finally block
     * 
     * @param <T>
     *            the visitor type
     * @param visitor
     ...
     * @return the visitor
     */
    public <T extends IResultSetVisitor> T executeQueryInSingleTransaction(final T visitor, final String querySql, final Object... params)
    {
