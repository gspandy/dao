...
public enum Databases implements IDatabaseVendor
{
	...
	{
		@Override
		public DbConnectionFactory init(final DbConnectionFactory connectionFactory,
	...
	{
		final DbConnectionFactory factory = DbConnectionFactory.getInstance();
		return init(factory, IDatabaseVendor.DEFAULT_CONNECTION_NAME, connectionDetails);
	}
	 ...
	 * 
	 * @param exp the exception used to initialise the error code class
	...
	@Override
	public abstract DbConnectionFactory init(final DbConnectionFactory connectionFactory,
			final String connectionName, final DbConnectionDetails details);
}