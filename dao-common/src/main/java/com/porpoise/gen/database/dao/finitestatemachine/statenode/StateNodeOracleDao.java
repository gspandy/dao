 ...
 * @author Aaron
 * */
class StateNodeOracleDao extends AbstractStateNodeDao
{
    public StateNodeOracleDao()
    {
        super();
    }

    public StateNodeOracleDao(final StateNode stateNode, 
    ...
    public StateNodeOracleDao(final Long id)
    {
        super(id);
    }
     ...
     )
    {
        ...
        final FirstResultToMapVisitor visitor = transaction.executeQuery(Visitors.firstResultVisitor(),
                "SELECT IDENTITY_VAL_LOCAL() FROM abp_STATE_NODE");
