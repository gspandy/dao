 ...
 *
 *
 * An implementation of a custom query -- a query with place-holders instead of
 * JDBC-style questionmarks
 */
public class CustomQueryImpl extends AbstractCustomQuery implements Serializable
{
    ...
    /**
     * @param create an unnamed custom query with the given SQL
     */
     ...
     *            the query sql
     */
    public CustomQueryImpl(Long id, final String nm, final String desc, final String sql)
    {
        setId(id);
        setName(nm);
        setDescription(desc);
     ...
     * 
     * @see com.porpoise.gen.query.ICustomQuery#executeQuery(com.porpoise.gen.database.IDbTransaction,
     *      com.porpoise.gen.database.IResultSetVisitor)
     */
    ...
    /**
     * {@link #executeQuery(IDbTransaction, IResultSetVisitor)} is the
     * preferable way to execute a custom query, though this method may be used
     * to return the calculated JDBC parameters
     * 
     * @return the JDBC parameters to be used for question-mark place-holders in
     *         a JDBC query
     */
     ...
     * {@link #executeQuery(IDbTransaction, IResultSetVisitor)} is the
     * preferable way to execute a custom query, though this method may be used
     * to return a JDBC query using questionmarks instead of
     * &quot;:placeholder&quot; style tokens
     * 
    ...
    {
        int hash = 37;
        hash = Objects.hash(getName(), getDescription(), getQuerySql());
        return hash;
    }
    ...
    /**
     * <p>
     * given the query string and user parameters, transform into a JDBC-style
     * query with question-mark place-holders. As the place-holders may be used
     * multiple times in the query, the size of the list returned may not be the
     * same size as the collection parameter.
     * </p>
     * <p>
     ...
     * 
     * @param query
     *            the query string with place-holders to convert into a
     *            JDBC-style query
     * @param sourceParams
     *            the user parameters
     * @return a list of user query parameters in the order they are used
     */
    static final List<UserQueryParameter> orderByKey(final String query,
            final Collection<UserQueryParameter> sourceParams)
    {
        final Map<Integer, UserQueryParameter> workingParamsByIndex = new HashMap<Integer, UserQueryParameter>(
                sourceParams.size());
        int lastIndex = -1;
