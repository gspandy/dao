...
public class DefaultCustomQueryService implements ICustomQueryService
{
    ...
    static final AaronLog                  LOG = AaronLog.getLog(DefaultCustomQueryService.class);
    ...
    static class QueryLoader extends AtLeastOneResultSetVisitor
    {
        ...
        public boolean onResultSet(final ResultSet resultSet) throws SQLException
        {
            ...
            final CustomQueryImpl customQuery = new CustomQueryImpl(id,
                    name,
                    desc,
                    sql);

            LOG.info("loading " + customQuery);
    ...
    static class QueryParamLoader extends AbstractResultSetVisitor
    {
        ...
        public boolean onResultSet(final ResultSet resultSet) throws SQLException
        {
            ...
            final UserQueryParameter param = new UserQueryParameter(key,
                    defaultValue);
    ...
    {
        this.factory = Validation.notNull(connectionFactory);
    }
    ...
    public void delete(final ICustomQuery customQuery)
    {
        ...
        LOG.info("deleting " + customQuery);
        final IDbTransaction transaction = this.factory.startNewTransaction();
        try
        {
            transaction.executeUpdate("DELETE FROM CUSTOM_QUERIES WHERE ID=?",
                    customQuery.getId());
            transaction.executeUpdate("DELETE FROM QUERY_PARAMETERS WHERE QUERY_ID=?",
                    customQuery.getId());
            transaction.commit();
        }
        finally
    ...
    private CustomQueryImpl get(final IDbTransaction transaction, final Long id)
    {
        ...
        final QueryLoader queryLoader = transaction.executeQuery(new QueryLoader(),
                "SELECT ID, NAME, DESCRIPTION, QUERY_SQL FROM CUSTOM_QUERIES WHERE ID=?",
                id);

        final QueryParamLoader paramLoader = transaction.executeQuery(new QueryParamLoader(),
                "SELECT PARAM_KEY, PARAM_NAME, DEFAULT_VALUE FROM QUERY_PARAMETERS WHERE QUERY_ID=?",
                id);
        ...
        try
        {
            transaction.executeQuery(new AbstractResultSetVisitor() {
                ...
                public boolean onResultSet(final ResultSet resultSet) throws SQLException
                {
                    ...
                    final CustomQueryImpl loadedQuery = get(transaction,
                            Long.valueOf(id));
        ...
        try
        {
            ...
            if (customQuery.getId() != null)
            {
                final SingleScalarResultVisitor visitor = transaction.executeQuery(Visitors.scalarVisitor(),
                        "SELECT COUNT(*) FROM CUSTOM_QUERIES WHERE ID=?",
                        customQuery.getId());
                id = visitor.getLong();
            ...
            if (create)
            {
                LOG.info("creating new " + customQuery);
                create(transaction, customQuery);
            }
            else
            {
                LOG.info("updating " + customQuery);
    ...
    private void create(final IDbTransaction transaction,
            final ICustomQuery customQuery)
    {
        transaction.executeUpdate("INSERT INTO CUSTOM_QUERIES(NAME, DESCRIPTION, QUERY_SQL) VALUES (?,?,?)",
                customQuery.getName(),
     ...
     */
    private void update(final IDbTransaction transaction,
            final ICustomQuery customQuery)
    {
     ...
     */
    private void insertUserParameters(final IDbTransaction transaction,
            final ICustomQuery customQuery)
    {
        for (final UserQueryParameter param : customQuery)
        {
            transaction.executeUpdate("INSERT INTO QUERY_PARAMETERS(QUERY_ID, PARAM_KEY, PARAM_NAME, DEFAULT_VALUE) "
                    + "VALUES (?,?,?,?)",// 
                    customQuery.getId(), //
                    param.getKey(), //
